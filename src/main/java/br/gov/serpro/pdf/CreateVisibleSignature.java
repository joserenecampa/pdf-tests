...
public class CreateVisibleSignature extends CreateSignatureBase {
	...
	public void signPDF(File inputFile, File signedFile) throws IOException {
		this.signPDF(inputFile, signedFile, null);
	}
	 ...
	 * @throws IOException
	 */
	public void signPDF(File inputFile, File signedFile, String signatureFieldName)
			throws IOException {
		...
		if (isExternalSigning()) {
			...
			// If you want to add the signature in a separate step, then set an
			// empty byte array
			// and call signature.getByteRange() and remember the offset
			// signature.getByteRange()[1]+1.
			// you can write the ascii hex signature at a later time even if you
			// don't have this
			// PDDocument object anymore, with classic java file random access
			// methods.
			...
			// PODDocument.getLastSignatureDictionary() and get the ByteRange
			// from there.
			// Close the file and then write the signature as explained earlier
			// in this comment.
			if (isLateExternalSigning()) {
				...
				// set signature bytes received from the service and save the
				// file
			...
			public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
				for (Callback callback : callbacks)
					if (callback instanceof PasswordCallback)
						((PasswordCallback)callback).setPassword("qtronix01".toCharArray());
			}
		});
		KeyStore keystore = loader.getKeyStore(); //KeyStore.getInstance("PKCS12");
		
		char[] password = "qtronix01".toCharArray(); //args[1].toCharArray(); // TODO use Java 6
...
//		keystore.load(new FileInputStream(args[0]), password);
		...
		int page = 1;
		signing.setVisibleSignDesigner(args[2], 100, 600, -75, imageStream, page);
		imageStream.close();
		signing.setVisibleSignatureProperties("José René Campanário", "Curitiba-PR, Brasil", "Porque o chefe pediu!!", 0, page, true);
